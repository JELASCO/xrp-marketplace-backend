...
app.use(helmet());
app.use(cors({ origin: [process.env.FRONTEND_URL, 'https://xrpharbor.com'].filter(Boolean), credentials: true }));
...
async function migrate() {
  ...
  try {
    ...
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar_url TEXT`);
      await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS notification_prefs JSONB DEFAULT '{"new_order":true,"order_completed":true,"dispute_opened":true,"dispute_resolved":true,"new_review":true}'::jsonb`);
