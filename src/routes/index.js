...
router.delete('/orders/:id', auth, async (req, res) => {
  try {
    const r = await db.query("DELETE FROM orders WHERE id = $1 AND buyer_id = $2 AND status IN ('pending','escrow_locked') RETURNING id", [req.params.id, req.user.id]);
    if (!r.rows[0]) return res.status(404).json({ error: 'Order not found or cannot be deleted' });
    res.json({ ok: true, deleted: r.rows[0].id });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/orders/cleanup-pending', auth, async (req, res) => {
  try {
    const r = await db.query("DELETE FROM orders WHERE buyer_id = $1 AND status IN ('pending','escrow_locked') RETURNING id", [req.user.id]);
    res.json({ ok: true, deletedCount: r.rows.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
