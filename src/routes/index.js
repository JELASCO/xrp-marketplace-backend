...
router.get('/me/stats', auth, async (req, res) => {
  try {
    ...
    res.json({
      activeListings: Number(ar.rows[0].cnt),
      soldListings: Number(sr.rows[0].cnt),
      completedSales: Number(cr.rows[0].cnt),
      totalRevenueXrp: Number(tr.rows[0].total),
      inEscrow: Number(er.rows[0].cnt),
      completedPurchases: Number(pr.rows[0].cnt),
      openDisputes: Number(dr.rows[0].cnt),
      reviewCount: Number(rr.rows[0].cnt),
      avgRating: Number(rr.rows[0].avg)
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
